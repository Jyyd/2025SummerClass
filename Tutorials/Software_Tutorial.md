...
> 指导老师: 张小乐 & 助教团队: 蹇姚宇蝶 董强（测试） 成宇恒（测试） 张嘉乐（测试）
> 2025年6月22日
...
* 如果安装时没有将 Anaconda 添加到环境变量，需要手动设置。
  ...
  * **Windows**：
    ...
    2. 在“系统变量”中找到 Path，点击“编辑”，添加 Anaconda 安装路径下的 `condabin` 和 `Scripts` 文件夹（如：`C:\Users\用户名\Anaconda3\condabin`）。
    3. 点击“确定”保存。
  * **Mac/Linux**：
    1. 打开终端，输入
       ```bash
...
3. **解压并放置文件：**
   ...
   * 将整个 `esp32` 文件夹，**复制到以下路径**（注意不同电脑用户名和路径可能不同）：
     ...
     ```
     C:\Users\***\AppData\Local\Arduino15\packages\expressif\esp32
     ```

     > 其中 `***` 为你的电脑登录用户名（或管理员名）。

   * 如果 `packages`、`expressif` 或 `esp32` 文件夹不存在，可以手动创建。
...
4. **检查结构：**
   ...
   * 正确路径下应为：
     ...
     ```
     C:\Users\你的用户名\AppData\Local\Arduino15\packages\expressif\esp32\hardware
     C:\Users\你的用户名\AppData\Local\Arduino15\packages\expressif\esp32\tools
     ```
...
1. 安装完成后，点击 `工具(Tools)` > `开发板(Board)`，在下拉菜单中选择你所用的 ESP32 板型（如 `ESP32 Dev Module`）。
...
1. 点击菜单栏 `工具(Tools)` > `库管理器(Library Manager)`。
2. 在右上角搜索对应库名，例如 `ArduinoJson`，点击 **安装**。
3. 依次搜索并安装其他所需库。
